...
async function sendBurnScheduleFileToTelegram() {
  ...
  try {
    ...
    const filePath = "/mnt/data/Gtg_burn_schedule.txt";  // your created file
...
const logPath = path.join(logDir, "burn-Gtg.json");
        ...
        if (eligible.length === 0) {
          ...
          if (allocatedTotal > airdropTotalBase) {
            ...
            airdropDetailsLines = perHolderShares.map((h) => {
              ...
              return `• \`${h.owner}\` — ${amtStr} Gtg`;
            });

            // Build full airdrop file text (CSV-like)
            const fileLines = [];
            fileLines.push(`Gtg Airdrop Distribution - ${formatTorontoTime()}`);
            fileLines.push(`Total airdrop: ${AIRDROP_TOTAL_UI.toLocaleString("en-US")} Gtg`);
            fileLines.push(`Min holding: ${MIN_HOLD_UI.toLocaleString("en-US")} Gtg`);
            fileLines.push(`Max holding: ${MAX_HOLD_UI > 0 ? MAX_HOLD_UI.toLocaleString("en-US") + " Gtg" : "No max"}`);
            fileLines.push("");
...
// 🔥 Send Gtg WARRIOR image to Telegram before airdrop starts
if (TG_BOT_TOKEN && TG_CHAT_ID) {
  try {
...
const scheduleUrl = "https://raw.githubusercontent.com/gtgdeveloper/gx/main/akio-burn/gtg_burn_schedule.txt";
...
const burnVideos = [
  ...
  "video7.mp4"
    ...
    if (airdropSummary) {
      ...
      const header =
        `🎁 *Airdrop Event*\n` +
        `Now for the fun part... we are airdropping hourly *${AIRDROP_TOTAL_UI.toLocaleString("en-US")}* Gtg to holders with minimum of  *${rangeText}* tokens.\n` +
