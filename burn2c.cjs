...
if (TG_BOT_TOKEN && TG_CHAT_ID) {
  try {
    const res = await fetch(`https://api.telegram.org/bot${TG_BOT_TOKEN}/sendDocument`, {
      ...
      body: JSON.stringify({
        chat_id: TG_CHAT_ID,
        document: "https://raw.githubusercontent.com/gtgdeveloper/gx/main/akio_burn_schedule.txt",
        caption: "📄 *AKIO DAILY BURN SCHEDULE*\n\nDownload the full burn plan.",
        parse_mode: "Markdown"
      })
    });
...
const burnVideos = [
  ...
  "video7.mp4"
    ...
    await sendTelegramVideo(chosenBurnVideo, "Now a word from our sponsor Avvenire.com");
    ...
    if (airdropSummary) {
      ...
      const header =
        `🎁 *Airdrop Event*\n` +
        `Now for the fun part... we are airdropping *${AIRDROP_TOTAL_UI.toLocaleString("en-US")}* AKIO to holders between *${rangeText}* tokens.\n` +
...
//      await sendTelegramMessage(airdropTgText);
      ...
      await sendTelegramVideo("video1b.mp4", "AKIO Airdrop Event");
    }
    // --- LOG JSON ---
